...
async def file_read(request: FileReadRequest):
    agent = HermesCodingAgent()
    res = await agent.execute(AgentContext(), tool="file_read", file_path=request.filePath)
    if "error" in res:
...
async def file_write(request: FileWriteRequest):
    agent = HermesCodingAgent()
    res = await agent.execute(AgentContext(), tool="file_write", file_path=request.filePath, content=request.content)
    if "error" in res:
        raise HTTPException(status_code=400, detail=res["error"])
    return res
...
async def terminal(request: TerminalRequest):
    agent = HermesCodingAgent()
    res = await agent.execute(AgentContext(), tool="terminal", command=request.command, args=request.args)
    if "error" in res:
        raise HTTPException(status_code=400, detail=res["error"])
    return res
